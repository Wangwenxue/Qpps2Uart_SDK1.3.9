                                   ...
                                   ke_task_id_t const src_id)
{
    ...
    if (get_bit_num(app_qpps_env->char_status) >= 1)
	{
		if(app_qpps_env->tx_buffer_available)
		{
			...
			{
				ble_data_tx();	// ble data tx  // wenxue 发送完成之后紧接着继续发送
			}
			else // pt_env.ble_tx_state != BLE_TX_GOING  或者 pt_data_fifo.state == PT_FIFO_EMPT
			{
				// 不再调用ble_data_tx()发送数据了
...
//		
//    uart_tx_buf_len = param->length;
//	  memcpy(uart_tx_buf,param->data,uart_tx_buf_len);
//    if(flag == 0)
//    {
//        flag = 1;
//        uart_write(QN_UART0, uart_tx_buf, uart_tx_buf_len, uart_tx_cfm);
//    }
    return (KE_MSG_CONSUMED);
}
...
//	if(app_qpps_env->tx_buffer_available)
	{
		if(pass_uart_env.length > 0)
		{
			app_qpps_env->tx_buffer_available--;
			
			if(pass_uart_env.length > 20)
			{
				app_qpps_data_send(app_qpps_env->conhdl, 0, 20, pass_uart_env.p_buf);
				pass_uart_env.p_buf += 20;
				pass_uart_env.length -= 20;			
			}
			else 
			{
				app_qpps_data_send(app_qpps_env->conhdl, 0, pass_uart_env.length, pass_uart_env.p_buf);
				//pass_uart_env.length = 0;
				//pass_uart_env.p_buf = pass_uart_env.buf_rx;
				pass_uart_rx_init();
			}
		}
	}
