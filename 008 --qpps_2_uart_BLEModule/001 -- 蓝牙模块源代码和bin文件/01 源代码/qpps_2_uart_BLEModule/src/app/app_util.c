...
uint8_t app_set_scan_rsp_data(uint16_t srv_flag)
{
    ...
    {
			// added by wenxue 2016-10-15
			// Len+ type+6bytes Mac+ user data
			 app_env.scanrsp_data[0] = pt_env.user_param.adv_user_data_len+6+ 1; // Length
       app_env.scanrsp_data[1] = GAP_AD_TYPE_MANU_SPECIFIC_DATA;
			
			 memcpy(app_env.scanrsp_data + 2, (char *)&pt_env.user_param.mac_address[0], 0x6);
			 len += 6; // 前面赋值了 len = 2;
			
		   // ADV user Data
		  if((pt_env.user_param.adv_user_data_len >= 1)&&(pt_env.user_param.adv_user_data_len <= 23))
		  {
			 memcpy((char *)&app_env.scanrsp_data[len], pt_env.user_param.adv_user_data, pt_env.user_param.adv_user_data_len);
			
			 len = len + pt_env.user_param.adv_user_data_len;
		  }
			
			app_env.scanrsp_data[len+0] = ATT_UUID_16_LEN + 1;
			app_env.scanrsp_data[len+1] = GAP_AD_TYPE_MORE_16_BIT_UUID;
			memcpy(app_env.scanrsp_data+len+2, QPP_SVC_PRIVATE_UUID, ATT_UUID_16_LEN);
			len = len+4;
			return (len); // 1+1+6+pt_env.user_param.adv_user_data_len+1+1+2
