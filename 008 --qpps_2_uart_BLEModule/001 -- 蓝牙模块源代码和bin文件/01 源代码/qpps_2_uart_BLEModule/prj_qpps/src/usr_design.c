...
void app_task_msg_hdl(ke_msg_id_t const msgid, void const *param)
{
    ...
    {  
        case GAP_SET_MODE_REQ_CMP_EVT:
            if(APP_IDLE == ke_state_get(TASK_APP)) // wenxue 表示第一次调用开始广播函数后
            {
							  app_pt_task_msg_hdl(msgid, param); // wenxue
                usr_led1_set(LED_ON_DUR_ADV_FAST, LED_OFF_DUR_ADV_FAST); // debug 用 后期可以去掉
               // ke_timer_set(APP_ADV_INTV_UPDATE_TIMER, TASK_APP, 30 * 100);
            }
            else if(APP_ADV == ke_state_get(TASK_APP))// 表示在广播过程中又调用广播函数
            {
                usr_led1_set(LED_ON_DUR_ADV_SLOW, LED_OFF_DUR_ADV_SLOW);
            }
            break;

        case GAP_ADV_REQ_CMP_EVT: // 广播停止 stop advertising has completed.
            usr_led1_set(LED_ON_DUR_IDLE, LED_OFF_DUR_IDLE);
            ke_timer_clear(APP_ADV_INTV_UPDATE_TIMER, TASK_APP);
			    	app_pt_task_msg_hdl(msgid, param);// wenxue
            break;

        case GAP_DISCON_CMP_EVT:  // 停止连接
            usr_led1_set(LED_ON_DUR_IDLE, LED_OFF_DUR_IDLE);
...
//                    GAP_ADV_FAST_INTV1, GAP_ADV_FAST_INTV2);
        ...
        case GAP_LE_CREATE_CONN_REQ_CMP_EVT: // inform the application the outcome of connection establishment
            if(((struct gap_le_create_conn_req_cmp_evt *)param)->conn_info.status == CO_ERROR_NO_ERROR)
            {
                if(GAP_PERIPHERAL_SLV == app_get_role())
                {
                    ...
                    //if (((struct gap_le_create_conn_req_cmp_evt *)param)->conn_info.con_interval >  IOS_CONN_INTV_MAX)
                    {
                        ...
                        //conn_par.intv_min = 6;
											conn_par.intv_min = pt_env.user_param.conn_intv_min; // wenxue
        ...
        case QPPS_CFG_INDNTF_IND:
	  	 ...
	  	 case GAP_READ_RSSI_REQ_CMP_EVT:
			     app_pt_task_msg_hdl(msgid, param);// wenxue
           break;
...
#endif
	 // wenxue 20161010
	 uart_init(PT_UART_PORT, USARTx_CLK(0), (enum UART_BAUDRATE)pt_env.user_param.baudrate);
   uart_tx_enable(PT_UART_PORT, MASK_ENABLE);
   uart_rx_enable(PT_UART_PORT, MASK_ENABLE);
	 timer_init(TX_WAKEUP_TIMER, timer0_callback);
	 
	 if(pt_env.rx_state == PT_RX_RAW_START) // 进入低功耗前的状态是raw rata
	 {
			pt_rx_raw_start();
	 }
	 if(pt_env.rx_state == PT_RX_PDU_STATR)// 进入低功耗前的状态pdu rata
	 {
		pt_rx_pdu_start();
	 }
...
/**
 ****************************************************************************************
 * @brief   BT_EN low callback
 * @description
 *   BT_EN is used to enter adv mode.
 ****************************************************************************************
 */
...
/**
 ****************************************************************************************
 * @brief   RX_WAKEUP callback
 * @description
 *   RX_WAKEUP is used to enter adv mode.
 ****************************************************************************************
 */
void gpio_rxwakeup_cb(void)
{
	    // If BLE is in the sleep mode, wakeup it.
    if(ble_ext_wakeup_allow())
    {
    ...
    // After wakeup BLE, the timer is not calibrated immediately and it is not precise.
    // So We set a event, in the event handle, set the soft timer.
    ke_evt_set(1UL << EVENT_GPIO_UART_RX_WAKEUP_ID);
        ...
        case BUTTON1_PIN:
            ...
            break;
				...
				case PT_STATE_CHANGE:
					  //BT_EN is used to enter adv mode, change state
		         gpio_stchange_cb();
		         break;
        case PT_UART_RX_WAKEUP: 
				  	//BT_EN is used to rx wakeup
			       gpio_rxwakeup_cb();
		         break;
...
//    }
		...
		module_init();// wenxue  BLE Module init
}
