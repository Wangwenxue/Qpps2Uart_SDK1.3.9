 ...
 ****************************************************************************************
 */
#define RX_FRAME_LEN 1
uint8_t g_ucRxdata[RX_FRAME_LEN];
void uart_rx_callback();
void uart_tx_callback();
 /*!
 * @brief uart_rx callback function
 */
 void uart_rx_callback()
 {
   gpio_uart_write(g_ucRxdata,RX_FRAME_LEN,uart_tx_callback);
 }   
   
/*!
 * @brief uart_tx callback function
 */
 void uart_tx_callback()
 {
    /*Start a data reception.*/ 
    gpio_uart_read(g_ucRxdata, RX_FRAME_LEN, uart_rx_callback);
   //return ;
 }  
...
int main(void)
{
    ...
    gpio_uart_read(g_ucRxdata, RX_FRAME_LEN, uart_rx_callback);
		 ...
		 /*Start a data transmision.*/ 
    gpio_uart_write("Gpio Simulate Uart demo start", sizeof("Gpio simulate Uart demo start"),uart_tx_callback);
