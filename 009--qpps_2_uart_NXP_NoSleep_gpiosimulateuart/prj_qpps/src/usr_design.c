...
// wenxue
#define GPIO_UART_RX_PIN    (GPIO_P31)
        ...
        case GAP_LE_CREATE_CONN_REQ_CMP_EVT:
            if(((struct gap_le_create_conn_req_cmp_evt *)param)->conn_info.status == CO_ERROR_NO_ERROR)
            {
                if(GAP_PERIPHERAL_SLV == app_get_role())
                {
					...
					//pt_rx_raw_start();	// fifo init, uart rx init
					//pt_enable();		// enable
                }
            }
            break;
    ...
    switch(pin)
    {
        ...
        case QN_TEST_CTRL_PIN:
            ...
            break;
				...
				// wenxue added by wenxue 12/7/2016
       case GPIO_UART_RX_PIN:
            // go to BOARD_GPIO_UART_RXPIN_HANDLER 
            BOARD_GPIO_UART_RXPIN_HANDLER(pin);
            break;
        default:
            break;
    }
...
struct app_pass_env_tag pass_uart_env;
static void pass_uart_rx_head_done(void);	
static void pass_uart_rx_data_done0(void);
static void pass_uart_rx_data_done1(void);
void pass_uart_rx_init()
{
    pass_uart_env.len_H = 0;
    pass_uart_env.len_L = 0;
	pass_uart_env.length = 0;
	pass_uart_env.length_backup = 0;
	pass_uart_env.ble_tx_pack_max = 0;
	pass_uart_env.ble_tx_pack_ct = 0;
	pass_uart_env.uart_flag = 0;
	pass_uart_env.p_uart_rx = pass_uart_env.buf_rx;
	pass_uart_env.p_ble_send = pass_uart_env.buf_rx;
	memset(pass_uart_env.buf_rx,0,DATA_BUF_LEN);
    uart_read(QN_UART0, &pass_uart_env.head, 3, pass_uart_rx_head_done);
}
void usr_init(void)
{
...
void pass_uart_rx_head_done()
{
	if(pass_uart_env.head == 0xAD)
	{
        pass_uart_env.length = (uint16_t)(pass_uart_env.len_H)*256 + pass_uart_env.len_L;
		pass_uart_env.length_backup = pass_uart_env.length;
		if(pass_uart_env.length%20)
			pass_uart_env.ble_tx_pack_max = (pass_uart_env.length/20) + 1;
		else
			pass_uart_env.ble_tx_pack_max = (pass_uart_env.length/20);
		pass_uart_env.uart_flag = 1;
		if(pass_uart_env.length <= 20)
		{
			uart_read(QN_UART0, pass_uart_env.buf_rx, pass_uart_env.length, pass_uart_rx_data_done0);
		}
		else
		{
			uart_read(QN_UART0, pass_uart_env.buf_rx, 20, pass_uart_rx_data_done1);
		}
	}
	else
	{
...
void pass_uart_rx_data_done1()
{
	if(pass_uart_env.length > 20)
	{
		pass_uart_env.length -= 20;
		pass_uart_env.p_uart_rx += 20;
		uart_read(QN_UART0, pass_uart_env.p_uart_rx, 20, pass_uart_rx_data_done1);
	}
	else
	{
		pass_uart_env.p_uart_rx += pass_uart_env.length;
		uart_read(QN_UART0, pass_uart_env.p_uart_rx, pass_uart_env.length, pass_uart_rx_data_done0);
	}

	if(pass_uart_env.uart_flag == 2)
	{
		if(pass_uart_env.p_ble_send < pass_uart_env.p_uart_rx)
		{
			pass_uart_env.uart_flag = 0;
			if(pass_uart_env.ble_tx_pack_ct < (pass_uart_env.ble_tx_pack_max))
			{
				app_qpps_env->tx_buffer_available--;
				app_qpps_data_send(app_qpps_env->conhdl, 0, 20, pass_uart_env.p_ble_send);
				pass_uart_env.p_ble_send += 20;
				pass_uart_env.ble_tx_pack_ct ++;
				pass_uart_env.length_backup -= 20;
			}
			else
			{
				app_qpps_env->tx_buffer_available--;
				app_qpps_data_send(app_qpps_env->conhdl, 0, pass_uart_env.length_backup, pass_uart_env.p_ble_send);
				pass_uart_env.ble_tx_pack_ct ++;
				//pass_uart_rx_init();
			}
		}
	}
	if(pass_uart_env.uart_flag == 1)
	{
		pass_uart_env.uart_flag = 0;
...
void pass_uart_rx_data_done0()
{

	if(pass_uart_env.uart_flag)
	{
		pass_uart_env.uart_flag = 0;
		if(app_qpps_env->tx_buffer_available)
		{
			app_qpps_env->tx_buffer_available--;
			app_qpps_data_send(app_qpps_env->conhdl, 0, pass_uart_env.length, pass_uart_env.p_ble_send);
			pass_uart_env.ble_tx_pack_ct ++;
		}
	}
//	pass_uart_rx_init();
